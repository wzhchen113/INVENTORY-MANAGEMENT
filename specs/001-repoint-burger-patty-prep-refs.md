...
- [x] **(AC7b — HTTP path through `pwa-catalog`)** Retroactively verified 2026-05-06 after the bind-mount fix landed via [specs/002-pwa-catalog-bind-mount-fix.md](002-pwa-catalog-bind-mount-fix.md). Local stack rebooted clean from repo root → mount source confirmed `<repo>/supabase/functions/` (not a worktree path); pwa-catalog migration replayed against seeded local DB (Path B-revised: 3 deleted, 1 repointed) **(prerequisite: a fresh `supabase start` reloads `seed.sql` which re-introduces the 4 orphans, so this AC requires re-applying the repoint migration against the now-seeded DB before verification — exact command run during build: `docker exec -i supabase_db_imr-inventory psql -U postgres -d postgres < supabase/migrations/20260504235959_repoint_burger_patty_orphans.sql`; see [specs/002-pwa-catalog-bind-mount-fix.md](002-pwa-catalog-bind-mount-fix.md) "Path B-revised migration replay" section under `## Build notes` for fuller context)**; HTTP `GET /functions/v1/pwa-catalog?store_id=<towson>` returned HTTP 200; `jq` subset filter `($referenced - $emitted)` over the 2AM Cheeseburger prep_items returned `[]`; stricter catalog-wide variant (every `recipes[].prep_items[]?.prep_recipe_id` minus `prep_recipes[].id`) also returned `[]`. Original block reason (stale `.claude/worktrees/pensive-raman-4d93c5/supabase/functions/` bind-mount returning `503 BOOT_ERROR`) was the subject of spec 002 and is now resolved.

...
export interface AppState {
  ...
  /**
   * Spec 120 — brand-scoped submission notification feed for the Cmd UI
   * bell. Named distinctly from `notifications` (the EOD-reminder inbox)
   * to avoid clobbering that pre-existing slice. Newest-first, capped at
   * the last 30 days / 50 rows server-side. `submissionUnreadCount` is the
   * badge source (an RPC anti-join count, not `filter(!read).length`, so it
   * stays correct even when the feed is capped). See spec 120 §6.
   */
  submissionNotifications: AdminNotification[];
...
/**
 * Spec 120 — brand-scoped submission notification (admin Cmd UI bell).
 * Distinct from `AppNotification` (the per-user EOD-reminder inbox): these
 * rows are generated by DB triggers on the 5 submission events, carry
 * brand/store/actor/type scoping, and track per-viewer read state via a
 * `notification_reads` join. The `read` flag is resolved per-viewer at
 * read time from the RLS-clipped embed of the caller's own read rows.
 * See spec 120 §5. Names are denormalized at trigger time (§1).
 */

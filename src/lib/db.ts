...
// ─── SUBMISSION NOTIFICATIONS (spec 120) ────────────────────────────────
// Brand-scoped submission feed for the admin Cmd UI bell. Distinct from the
// per-user EOD-reminder inbox above (`in_app_notifications`). Rows are
// generated by DB triggers on the 5 submission events (§0 corrected source
// map); RLS clips to the caller's brand scope (super_admin = all brands,
// admin = own brand) and denies non-privileged `user`s at the DB. Per-viewer
// read state lives in `notification_reads`; the `read` flag derives from the
// RLS-clipped embed of the caller's OWN read rows. All admin-facing
// reads/writes flow through here (staff has no bell). See spec 120 §5.
//
// NOTE (parallel build): backend-developer owns the matching migration
// (`20260715000000_submission_notifications.sql` — tables, RLS, triggers,
// and the two RPCs `unread_notification_count` / `mark_all_notifications_read`).
// These helpers are authored to the spec §5 signatures/query shapes; if
// backend also lands db.ts helpers, reconcile at merge (identical shapes).

/** Days-window for the feed + count (spec 120 §1 — last 30d / limit 50). */
const SUBMISSION_NOTIF_WINDOW_DAYS = 30;
const SUBMISSION_NOTIF_LIMIT = 50;
...
/**
 * Feed — RLS-scoped (super_admin = all brands, admin = own brand),
 * newest-first, last-30-days + limit 50. `read` derives from the
 * RLS-clipped embed of the caller's own notification_reads rows.
 */
export async function fetchAdminNotifications(
  opts?: { limit?: number },
): Promise<AdminNotification[]> {
  const limit = opts?.limit ?? SUBMISSION_NOTIF_LIMIT;
  const since = new Date(
    Date.now() - SUBMISSION_NOTIF_WINDOW_DAYS * 24 * 60 * 60 * 1000,
  ).toISOString();
  return useInflight.getState().track(async (signal) => {
    const { data, error } = await supabase
      .from('notifications')
      ...
      .abortSignal(signal);
    if (error) {
      console.warn('[Supabase] fetchAdminNotifications:', error.message);
      return [];
    }
    return (data || []).map(mapNotification);
  }, { kind: 'read', label: 'fetchAdminNotifications' });
}
...
export async function fetchUnreadNotificationCount(): Promise<number> {
  return useInflight.getState().track(async (signal) => {
    const { data, error } = await supabase
      ...
      .abortSignal(signal);
    if (error) {
      console.warn('[Supabase] fetchUnreadNotificationCount:', error.message);
      return 0;
    }
    return typeof data === 'number' ? data : Number(data ?? 0);
  }, { kind: 'read', label: 'fetchUnreadNotificationCount' });
}

/**
 * Mark one notification read for the current viewer. Inserts
 * `(notification_id)` (user_id defaults to auth.uid() server-side),
...
export async function markNotificationRead(id: string): Promise<void> {
  return useInflight.getState().track(async (signal) => {
    const { error } = await supabase
      ...
      .abortSignal(signal);
    if (error) {
      console.warn('[Supabase] markNotificationRead:', error.message);
      throw error;
    }
  }, { kind: 'write', label: 'markNotificationRead' });
}

/**
 * Mark all currently-scoped unread notifications read for the viewer —
 * RPC (SECURITY INVOKER). Returns the number of rows marked.
 */
export async function markAllNotificationsRead(): Promise<number> {
  return useInflight.getState().track(async (signal) => {
    const { data, error } = await supabase
      ...
      .abortSignal(signal);
    if (error) {
      console.warn('[Supabase] markAllNotificationsRead:', error.message);
      throw error;
    }
    return typeof data === 'number' ? data : Number(data ?? 0);

...
// Dynamic Schedule-Based Order Report — "I.M.R - Daily Replenishment Guide"
...
function exportToCSV(data: DynamicOrderLine[], vendorName: string, dateStr: string) {
  ...
  link.href = url;
  link.download = `IMR_Order_${vendorName.replace(/\s/g, '_')}_${dateStr}.csv`;
  link.click();
...
async function exportToPDF(data: DynamicOrderLine[], vendorName: string, storeName: string, orderDate: Date, daysToCover: number) {
  ...
  doc.text('Generated by I.M.R — Inventory Management for Restaurant', 14, pageHeight - 10);

  const isoDate = toInputDate(orderDate);
  doc.save(`IMR_Order_${vendorName.replace(/\s/g, '_')}_${isoDate}.pdf`);
}
  ...
  return (
    <View style={[styles.container, { backgroundColor: C.bgTertiary }]}>
      ...
      <View style={[styles.titleBar, { backgroundColor: C.textPrimary }]}>
        <View>
          <Text style={[styles.brandName, { color: C.bgPrimary }]}>I.M.R</Text>

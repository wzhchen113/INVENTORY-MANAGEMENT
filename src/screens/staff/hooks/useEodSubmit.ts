...
export function useEodSubmit(): {
  ...
  const pending = useStaffStore((s) => s.pendingCountForUser(userId));
  const draining = useStaffStore((s) => s.draining);
  ...
  const drain = useCallback(async () => {
    ...
    useStaffStore.getState().setDraining(true);
    try {
      const me = userId;
      if (!me) {
      ...
      // ever drifts.
      const snapshot = [...useStaffStore.getState().eodQueue].sort(
      ...
      for (const item of snapshot) {
        ...
        try {
          ...
          await useStaffStore.getState().dequeueEod(item.client_uuid);
          drainedAny = true;
          if (res.conflict) {
          ...
          if (isForbidden(err)) {
            ...
            await useStaffStore.getState().bumpEodAttempts(
              item.client_uuid,
              'network',
            );
            break;
      ...
      if (
        drainedAny &&
        useStaffStore.getState().pendingCountForUser(me) === 0
      ) {
      ...
      useStaffStore.getState().setDraining(false);
    }
  }, [userId]);
  ...
  const submit = useCallback(
    async (payload: SubmitPayload): Promise<Outcome> => {
      ...
      if (!isOnline) {
        await useStaffStore.getState().enqueueEod(queueItem);

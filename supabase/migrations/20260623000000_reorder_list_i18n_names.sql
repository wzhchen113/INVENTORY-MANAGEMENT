...
-- 20260602000000_reorder_suggested_cases.sql (the LATEST definition —
-- it carries spec 087's as_of_date / EOD-first logic AND spec 088's
-- case math). Per the function header rule, this `create or replace`
-- copies that latest body, NOT the spec-021-era revision. Exactly TWO
-- additive hunks here (surface the catalog's localized-name overrides
-- so the staff reorder screen can render item names the way EOD/Weekly
-- already do via getLocalizedName):
--
--   (1) per_item CTE: surface ci.i18n_names from the EXISTING `ci`
--       join — `ci.i18n_names as i18n_names`. No new join, no new
--       scan. The column rides through per_item_suggested (`pi.*`)
--       and per_item_filtered (`pis.*`) unchanged.
--
--   (2) per-item jsonb_build_object: add ONE additive key —
--       'i18n_names', pif.i18n_names — placed immediately after
--       'item_name' for readability. i18n_names is JSONB; a NULL
--       catalog value serializes to JSON `null`, which the staff
--       mapper coalesces to {} so getLocalizedName falls through
--       silently to the English `name`.
--
-- Behavior preserved: every other CTE, the par/forecast formula,
-- suggested_qty, the as_of_date/EOD logic, the schedule/next-delivery
-- math, the case math (spec 088), and the vendor filter are UNCHANGED.
-- Output is IDENTICAL except for the additive per-item i18n_names key.
--
-- Admin-consumer safety: both consumers of this RPC's output
-- (src/lib/db.ts:mapReorderVendor for the admin desktop, and the
-- verbatim copy at src/screens/staff/lib/fetchReorder.ts) map by NAMED
-- JSON fields and ignore unknown keys — the extra i18n_names key is
-- inert for the admin path (admin stays English, per spec scope).
--
...
-- ACL; re-stating it here would be redundant churn. Backward-compatible
-- (extra JSON key; an old client keeps working and just ignores it).
-- ============================================================

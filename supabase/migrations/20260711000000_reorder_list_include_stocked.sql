-- 20260711000000_reorder_list_include_stocked.sql
--
-- Spec (owner request, 2026-07): the staff Reorder screen shows ALL
-- ingredients for the day's vendors, split into "Needs to Order" (below par)
-- and "Have enough stock" (at/above par). The prior report_reorder_list
-- filtered at/above-par items out entirely (and dropped vendors with none),
-- so those items never reached the client.
--
-- Additive `create or replace` — SAME (uuid, jsonb) signature, ACL preserved.
-- Two changes, both gated so ADMIN is byte-for-byte unaffected:
--   1. `p_params.include_stocked` (bool, default false): when true, the
--      per_item_filtered CTE keeps every item instead of only suggested_qty
--      >= 0.001. Admin never passes it → original below-par-only behaviour.
--   2. A per-item `needs_order` boolean is always emitted (suggested_qty >=
--      0.001). Admin items are all below-par → all true → no visible change;
--      the staff client uses it to split the two sections + colour rows.
--
-- Base: the report_reorder_list definition from
-- 20260704000000_po_loop.sql (the latest redefinition — includes the spec-104
-- per-each cost basis), copied verbatim with ONLY those two edits.

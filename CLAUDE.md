...
- **Edge functions and SECURITY DEFINER RPCs performing destructive role-change or deletion operations enforce a server-side `caller.id != target.id` guard.** The refusal raises with stable, byte-for-byte refusal strings following the `'cannot <verb> self'` convention: `'cannot delete self'` ([supabase/functions/delete-user/index.ts:168-173](supabase/functions/delete-user/index.ts)) and `'cannot demote self'` ([supabase/migrations/20260520000000_demote_profile_to_user_rpc.sql](supabase/migrations/20260520000000_demote_profile_to_user_rpc.sql), spec 050). The SQL path uses SQLSTATE `P0001` so PostgREST maps to HTTP 400; the edge-function path returns HTTP 400 directly. The guard MUST fire BEFORE any side-effect (UPDATE / DELETE / `auth.admin.deleteUser`). A new destructive path that targets a profile id MUST add a self-guard before any side-effect, with a string that follows the convention. Sibling rule to the last-of-role guard — both protect against recover-by-psql-only states. Reference shapes: the spec 031 last-of-role bullet above and the spec 050 RPC migration. A future role-hierarchy spec (e.g. only super_admins can demote super_admins) extends this rule rather than replaces it.
...
- **Permissive RLS policies on the same `(table, command)` pair are ORed.** Postgres composes permissive policies via OR: a row is admitted iff ANY permissive policy's USING/WITH CHECK is satisfied. This means a single wide `using (auth.uid() IS NOT NULL)` policy on a table neutralizes every scoped permissive policy that exists alongside it for the same command — the scoped policies never get the chance to deny. When adding a permissive policy, check `pg_policies` for existing permissive policies on the same `(schemaname, tablename, cmd)` and either consolidate or convert to `RESTRICTIVE` if the new policy is meant to TIGHTEN rather than broaden. Spec 051 ([supabase/migrations/20260520010000_legacy_permissive_policy_dropout.sql](supabase/migrations/20260520010000_legacy_permissive_policy_dropout.sql)) audited four such legacy wide policies on `public.stores`, `public.user_stores`, `public.ingredient_categories`, and `public.recipe_categories` and dropped or rewrote each. Spec 053 ([supabase/tests/permissive_policy_lint.test.sql](supabase/tests/permissive_policy_lint.test.sql)) enforces this via a pgTAP CI probe that fails the build on any future permissive policy in `public.*` whose USING or WITH CHECK is trivially-wide (`auth.uid() IS NOT NULL`, `true`, `auth.role() = 'authenticated'`, or those tokens in an OR-tail) without an explicit allowlist entry. The allowlist seeds the two `*_categories` SELECT policies above; a future intentional cross-brand wide policy adds one row to the VALUES list in the same PR.
...
### CI status check after every push to `main`

After any push to `main` (whether by the user or by an agent acting on the user's behalf), the next agent action MUST confirm the most recent run of **both** active gates on `main` is green — [.github/workflows/test.yml](.github/workflows/test.yml) **and** [.github/workflows/db-migrations-applied.yml](.github/workflows/db-migrations-applied.yml) — via `gh run list --branch main --workflow <file> --limit 1` for each (or equivalent). If either run is red or in-progress, the agent surfaces the run URL and waits for user direction before continuing pipeline work. The `db-migrations-applied` gate is an **independent** signal from `test.yml`: it hard-fails when a repo migration is missing from prod's `schema_migrations` (committed to the repo but never `supabase db push`ed). On 2026-06-28 it sat red for days, unnoticed because only `test.yml` (green) was being checked, while `20260623000000_reorder_list_i18n_names` had never been pushed to prod — so a green `test.yml` alone is NOT sufficient evidence the branch is healthy. This catches the local-pgTAP-green / CI-pgTAP-red asymmetry that allowed spec 060's CI crash to go unnoticed for 7 runs / 3 days before spec 067 caught it. (One class of this asymmetry — a Supabase-CLI image silently revoking the default `public.*` grants — is durably fixed by spec 097's explicit-grant migration `20260618000000_public_grants_explicit.sql`.) `release-coordinator` MUST not recommend SHIP_READY when the latest run of **either** gate (`test.yml` or `db-migrations-applied.yml`) on `main` is not green (extends the existing "any Critical blocks SHIP_READY" hard rule). Chicken-and-egg note: if `test.yml` is currently red, the agent surfaces this, the user authorizes the fix PR, and the rule re-engages after the first post-fix green run.
